	...
	$.ajax({
		...
		success:function(resp)
		{
			...
			$.each( resp, function(key, result)
			{
				...
				var visitBtn = "<a id='visitBtn'><span class='badge badge badge-info' style='background-color: #0090ff'>New Visit</span></a>"
				$("#after_tr").after("<tr id="+result['patient_id']+"><td>"+result['patient_id']+"</td><td>"+result['patient_name_en']+"</td><td>"
					...
					+result['patient_status']+"</td><td><select style='color: #0090ff; ' class='form-control select patient_status' name='patient_status'><option value='select'>Select</option><option value='Active'>Active</option><option value='Deceased'>Deceased</option><option value='Discharged'>Discharged</option><option value='Defaulter'>Defaulter</option></select><td>"+visitBtn+"</td>");
				
				//if visit button clicked
				$("#patient_table #visitBtn").on('click', function(){
					var id = $(this).closest('tr').attr('id');
					window.location.href = "new_visit.php?pid="+id;
				})
